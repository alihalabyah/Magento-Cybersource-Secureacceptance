 ...
 */
class Cybersource_Secureacceptance_Model_Secureacceptance extends Mage_Payment_Model_Method_Abstract
{
    ...
    protected $_code 			= 'secureacceptance';
    protected $_formBlockType 	= 'secureacceptance/form';
    protected $_infoBlockType 	= 'secureacceptance/info';
	...
	public function validate()
    {
        ...
        if (!$this->canUseForCurrency($currencyCode)) {
            Mage::throwException(Mage::helper('secureacceptance')->__('Selected currency code ('.$currencyCode.') is not compatabile with this payment.'));
        }
        return $this;
    }
...
//        }
    ...
    {
          return Mage::getUrl('secureacceptance/process/redirect');
    }

    protected function getSuccessUrl()
	{
		return Mage::getUrl('secureacceptance/process/success', array('_secure' => true));
	}

	protected function getFailureUrl()
    {
        return Mage::getUrl('secureacceptance/process/failure', array('_secure' => true));
    }

    protected function getCancelUrl()
    {
        return Mage::getUrl('secureacceptance/process/cancel', array('_secure' => true));
    }

    protected function getIpnUrl()
    {
        return Mage::getUrl('secureacceptance/process/ipn', array('_secure' => true));
    }
	...
	{
		$hashSign = Mage::helper('secureacceptance')->getHashSign($formFields);
		return $hashSign;
	}
	...
	public function getFormFields()
	{
        ...
        if($this->getConfigData('debug_flag')){
            Mage::helper('secureacceptance')->log($formFields);//for debug purpose
            $resource       = Mage::getSingleton('core/resource');
            $connection 	= $resource->getConnection('core_write');
    	    $sql            = "INSERT INTO ".$resource->getTableName('secureacceptance_api_debug')." SET created_time = ?, request_body = ?, response_body = ?";
    	    $connection->query($sql, array(date('Y-m-d H:i:s'), $this->getSecureacceptanceUrl()."\n".print_r($formFields, 1), ''));
        }
